...
const LEDGER_TRANSACTION_DATA = 'TRANSACTION_DATA'
const WALLET = 'Ledger'
function transactionDataCheck(): any {
  return async (stateAndHelpers) => {
    const { wallet } = stateAndHelpers
    const isTransactionDataEnabled = await loadFromStorage(LEDGER_TRANSACTION_DATA)
    if (wallet && wallet.name === WALLET && !isTransactionDataEnabled) {
      return {
        ...
        button: {
          text: 'Done',
          onclick: async () => await saveToStorage(LEDGER_TRANSACTION_DATA, true),
        },
        icon: `
