...
const _isTxMined = async (sessionBlockNumber: number, txHash: string): Promise<boolean> => {
...
const monitorTx = async (
  ...
  return (
    backOff(() => PendingTxMonitor._isTxMined(sessionBlockNumber, txHash), options)
      .then((status) => {
        // Tx reverted
        if (status === false) clearPending()
      })
