...
const isTxHashSignedWithPrefix = (txHash: string, signature: string, ownerAddress: string): boolean => {
  let hasPrefix
  try {
    const rsvSig = {
...
export const adjustV: AdjustVOverload = (
  ...
  if (signingMethod === 'eth_sign') {
    /* 
      ...
      Some wallets do that, some wallets don't, V > 30 is used by contracts to differentiate between these two values
