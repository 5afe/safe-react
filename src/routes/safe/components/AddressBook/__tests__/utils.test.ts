...
describe('Address Book file validations', () => {
  it('It should return wrong file extension error if file extension is not the allowed', () => {
    const file = new File([''], 'file.txt', { type: 'text/plain' })
    const result = validateFile(file)
    expect(result).toBe(WRONG_FILE_EXTENSION_ERROR)
  })

  it('It should return file size error if file size is over the allowed', () => {
    const exampleFile = fs.readFileSync(`${__dirname}/wrongSizeFile.csv`)
    const file = new File([exampleFile.toString()], 'file.csv', { type: IMPORT_SUPPORTED_FORMATS[0] })
    const result = validateFile(file)
    expect(result).toBe(FILE_SIZE_TOO_BIG_ERROR)
  })

  it('It should return undefined if extension and file size are valid', () => {
...
describe('Address Book CSV data validations', () => {
  ...
  it('It should return an error if the amount of columns is not valid', () => {
