...
const ExpandedTx = ({
  ...
  const thresholdReached = !INCOMING_TX_TYPE.includes(tx.type) && threshold <= tx.confirmations.size
  const canExecute = !INCOMING_TX_TYPE.includes(tx.type) && nonce === tx.nonce
      ...
      <Block className={classes.expandedTxBlock}>
        <Row>
          <Col layout="column" xs={6}>
            <Block
              className={cn(classes.txDataContainer, INCOMING_TX_TYPE.includes(tx.type) && classes.incomingTxBlock)}
            >
              ...
              {INCOMING_TX_TYPE.includes(tx.type) ? (
                <>
                  <Paragraph noMargin>
                    <Bold>Created: </Bold>
            ...
            <Hairline />
            {INCOMING_TX_TYPE.includes(tx.type) ? <IncomingTxDescription tx={tx} /> : <TxDescription tx={tx} />}
          </Col>
          {!INCOMING_TX_TYPE.includes(tx.type) && (
            <OwnersColumn
