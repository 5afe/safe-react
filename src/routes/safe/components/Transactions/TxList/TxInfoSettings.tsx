...
export const TxInfoSettings = ({ settingsInfo }: TxInfoSettingsProps): ReactElement | null => {
  ...
  switch (settingsInfo.type) {
    ...
    case 'REMOVE_OWNER': {
      ...
      return (
        <InfoDetails title={title} header={<ThresholdWarning />}>
          ...
          />
          <InfoDetails title="Change the confirmation policy to:">{settingsInfo.threshold}</InfoDetails>
        </InfoDetails>
      )
    }
    case 'SWAP_OWNER': {
    ...
    case 'CHANGE_THRESHOLD': {
      return (
        <InfoDetails title="Change the confirmation policy to:" header={<ThresholdWarning />}>
