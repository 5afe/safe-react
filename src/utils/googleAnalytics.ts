...
import { currentNetwork, currentNetworkId } from 'src/logic/config/store/selectors'
...
export const trackAnalyticsEvent = (event: Parameters<typeof ReactGA.event>[0]): void => {
  const { chainName } = currentNetwork(store.getState())
...
export const loadGoogleAnalytics = (): void => {
  ...
  const { chainId } = currentNetwork(store.getState())
